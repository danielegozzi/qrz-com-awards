...
dxcc = {
  "dxcc": [
    ...
    },
    {
      ...
      "notes": "",
      "outgoingQslService": true,
      "prefix": "JA,JB,JC,JD,JE,JF,JG,JH,JI,JJ,JK,JL,JM,JN,JO,JP,JQ,JR,JS,7J,7K,7L,7M,7N",
...
var country_names = award_100_table.querySelectorAll("td.lent span");
var country_status = award_100_table.querySelectorAll("td.lstat");

function assemble_regexes_for_wsjtx_log(regexes) {
  return " +(?:" + regexes.map((r) => r.slice(1, r.length -1)).join("|") + ") +[A-Z0-9+-]+$"
}
...
for (let cn of country_names.entries()) {
  ...
  if (country_name == "United States") {
    country_name += " of America"; // ;-)
  }
  cs = country_status[i].innerHTML;
  min_distance = 1000;
  min_dixtance_idx = -1;
  ...
  if (cs == "Confirmed") {
    dxcc.dxcc = dxcc.dxcc.slice(0, min_distance_idx).concat(dxcc.dxcc.slice(min_distance_idx+1))
  }
}
